...
def encode(tokenized_x):
    """Encode samples and pad."""
    new_sent = [] 
...
def scrub(samples):
    ...
    for sample in samples:
        new_samples.append(encode(tokenize(sample)))
    return new_samples
...
trainer.train(get_dataloader(samples, results, TEST_CUTOFF, MAX_LEN))

samples, results = load(test)
samples = scrub(samples[:TEST_CUTOFF])
trainer.eval(get_dataloader(samples, results, TEST_CUTOFF, MAX_LEN))
